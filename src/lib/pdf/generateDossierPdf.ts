...
export function generateDossierPdf(dossier: CompanyDossier): Buffer {
  ...
  w.paragraph(
    dossier.isDemoData
      ? "Установочные данные помечены источником ФНС со статусом «Данные получены» получены из реального ЕГРЮЛ (через DaData); остальные разделы этого отчёта — ДЕМОНСТРАЦИОННЫЕ и сгенерированы автоматически для целей MVP (см. статусы источников выше). Отчёт не является официальной справкой и не может использоваться как юридически значимый документ."
      : "Данные этого отчёта получены из подключённых источников (см. статусы выше). Отчёт не является официальной справкой и не может использоваться как юридически значимый документ.",
